with taxonomy_seed (
  ...
  values
    (
      'SELF_ASSESSMENT_SPORTS_primary_goal',
      'Self-assessment sports primary goal',
      array['self-assessment', 'sports', 'goal']::text[],
      'string',
      null,
      'Primary sports rehabilitation goal captured during sports self-assessment.'
    )
)
...
with semantic_seed (
  ...
  values
    (
      ...
      'string',
      null,
      'Primary rehabilitation goal captured during sports self-assessment.'
    )
)
...
select
  ...
  ss.default_unit,
  ss.notes,
  jsonb_build_object('seed', '2026-05-18', 'form_code', ss.form_code, 'wave', 'self_assessment_sports_primary_goal'),
  'active'
...
where lower(afr.form_template_id) = 'self_assessment_sports';
