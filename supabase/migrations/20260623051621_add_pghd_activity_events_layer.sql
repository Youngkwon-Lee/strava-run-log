...
comment on table public.pghd_activity_events is
  'Provider-originated PGHD activity event staging table. Generic source layer that feeds existing PhysioApp read models, context snapshots, memory chunks, clinical state, and timeline surfaces without replacing them.';
...
drop policy if exists pghd_activity_events_select on public.pghd_activity_events;
create policy pghd_activity_events_select
  on public.pghd_activity_events
  for select
  to authenticated
  using (
...
drop policy if exists pghd_activity_events_service_role_all on public.pghd_activity_events;
create policy pghd_activity_events_service_role_all
  on public.pghd_activity_events
  for all
  to service_role
