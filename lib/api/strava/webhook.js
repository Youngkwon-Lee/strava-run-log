import { getActivityDetail, normalizeActivity, refreshTokenIfNeeded, summarizeActivity } from '../../strava.js';
import { buildPostRunCoaching } from '../../coaching.js';
import { postDiscord } from '../../discord.js';
import { appendRunToPhysioKernel } from '../../physio-kernel.js';
import { upsertStoredRun } from '../../run-store.js';

const VERIFY_TOKEN = process.env.STRAVA_VERIFY_TOKEN;

export default async function handler(req, res) {
  try {
    if (req.method === 'GET') {
      const mode = req.query['hub.mode'];
      const token = req.query['hub.verify_token'];
      const challenge = req.query['hub.challenge'];
      if (mode === 'subscribe' && token === VERIFY_TOKEN) {
        return res.status(200).json({ 'hub.challenge': challenge });
      }
      return res.status(403).json({ error: 'verify failed' });
    }

    if (req.method === 'POST') {
      const evt = req.body || {};
      if (evt.object_type !== 'activity' || evt.aspect_type === 'delete') {
        return res.status(200).json({ ok: true, ignored: true });
      }
      const athleteId = String(process.env.STRAVA_ATHLETE_ID || '').trim();
      const subscriptionId = String(process.env.STRAVA_SUBSCRIPTION_ID || '').trim();
      if (!athleteId || !subscriptionId) throw new Error('missing Strava webhook scope configuration');
      if (
        String(evt.owner_id || '') !== athleteId ||
        String(evt.subscription_id || '') !== subscriptionId
      ) {
        return res.status(403).json({ error: 'webhook scope mismatch' });
      }

      const token = await refreshTokenIfNeeded();
      const detail = await getActivityDetail(evt.object_id, token);
      const s = summarizeActivity(detail);
      const normalized = normalizeActivity(detail);
      const coaching = buildPostRunCoaching(detail, {
        targetPaceSec: Number(process.env.COACH_TARGET_PACE_SEC || 370),
        targetRpe: process.env.COACH_TARGET_RPE || '6~7'
      });
      const kernelEvent = await appendRunToPhysioKernel({
        ...normalized,
        source: 'strava',
        provider: 'strava',
        externalId: normalized.id,
        coaching
      });
      await upsertStoredRun(
        {
          ...normalized,
          source: 'strava',
          provider: 'strava',
          externalId: normalized.id,
          coaching,
          subjectPersonId: kernelEvent.subjectPersonId,
          organizationId: kernelEvent.organizationId,
          pghdActivityEventId: kernelEvent.activityEventId
        },
        { persistPghdActivityEvent: false, resolveConnection: false, skipCount: true }
      );
      const extra = [
        normalized.averageHeartrate ? `평균 HR ${Math.round(normalized.averageHeartrate)}` : '',
        normalized.averageCadence ? `케이던스 ${Math.round(normalized.averageCadence)}` : '',
        normalized.calories ? `${Math.round(normalized.calories)}kcal` : '',
        normalized.deviceName || ''
      ].filter(Boolean);

      const txt = [
        '🏃 새 러닝 감지',
        `- ${s.title}`,
        `- ${s.km}km · ${s.moving} · ${s.pace} · 상승 ${s.elev}m`,
        extra.length ? `- ${extra.join(' · ')}` : '',
        '',
        coaching
      ].filter((line) => line !== '').join('\n');
      await postDiscord(txt);

      return res.status(200).json({ ok: true });
    }

    return res.status(405).json({ error: 'method not allowed' });
  } catch (e) {
    return res.status(500).json({ error: e.message });
  }
}
