...
function checkRunStoreBackend() {
  const rawBackend = String(process.env.RUN_STORE_BACKEND || 'file').trim().toLowerCase();
  const backend = rawBackend === 'jsonl' ? 'file' : rawBackend;
  const isVercel = Boolean(process.env.VERCEL);
  const ephemeralFileAllowed = envFlag(process.env.RUN_STORE_ALLOW_EPHEMERAL_FILE);
  const hasSupabaseUrl = Boolean(process.env.SUPABASE_URL);
  const hasSupabaseKey = Boolean(process.env.SUPABASE_SERVICE_ROLE_KEY || process.env.SUPABASE_SECRET_KEY);

  if (backend === 'supabase') {
    if (hasSupabaseUrl && hasSupabaseKey) {
      return check('run_store_backend', 'ok', 'Run history store is configured for durable Supabase storage.', {
        backend,
        durable: true,
        table: process.env.RUN_STORE_SUPABASE_TABLE || process.env.RUN_LOG_TABLE || 'run_log_runs'
      });
    }

    return check('run_store_backend', 'error', 'Run history store is set to Supabase but required Supabase env is missing.', {
      backend,
      durable: false,
      missingSupabaseUrl: !hasSupabaseUrl,
      missingSupabaseKey: !hasSupabaseKey
    }, ['Set SUPABASE_URL and SUPABASE_SERVICE_ROLE_KEY for the deployed run store.']);
  }

  if (backend === 'file') {
    if (isVercel && !ephemeralFileAllowed) {
      return check('run_store_backend', 'error', 'Vercel file storage is blocked because it would write run history to ephemeral /tmp.', {
        backend,
        durable: false,
        vercel: true,
        ephemeralFileAllowed: false
      }, ['Set RUN_STORE_BACKEND=supabase and configure SUPABASE_URL/SUPABASE_SERVICE_ROLE_KEY in deployment env.']);
    }

    if (isVercel && ephemeralFileAllowed) {
      return check('run_store_backend', 'warning', 'Vercel file storage is explicitly enabled for temporary smoke/dev use only.', {
        backend,
        durable: false,
        vercel: true,
        ephemeralFileAllowed: true
      }, ['Remove RUN_STORE_ALLOW_EPHEMERAL_FILE before production exposure and use RUN_STORE_BACKEND=supabase.']);
    }

    return check('run_store_backend', 'warning', 'File run history store is local-only and not durable enough for release/staging.', {
      backend,
      durable: false,
      vercel: false
    }, ['Set RUN_STORE_BACKEND=supabase before release/staging so ingested runs use durable storage.']);
  }

  return check('run_store_backend', 'error', `Unsupported RUN_STORE_BACKEND: ${rawBackend}.`, {
    backend: rawBackend,
    durable: false
  }, ['Use RUN_STORE_BACKEND=supabase for deployed PGHD environments.']);
}
