import { getStravaSession, setStravaSession } from './session.js';
...
function getActivityTime(activity) {
  const value = activity.startDate || activity.start_date || activity.startDateLocal || activity.start_date_local;
  const time = value ? Date.parse(value) : 0;
  return Number.isFinite(time) ? time : 0;
}

function getActivityDistanceMeters(activity) {
  return Number(activity.distanceMeters ?? activity.distance ?? 0);
}

function assertOAuthEnv() {
  if (!process.env.STRAVA_CLIENT_ID || !process.env.STRAVA_CLIENT_SECRET) {
    throw new Error('missing Strava OAuth env: STRAVA_CLIENT_ID, STRAVA_CLIENT_SECRET');
  }
}

async function postStravaOAuth(body) {
  assertOAuthEnv();
  ...
  if (!r.ok) {
    ...
    throw new Error(`Strava OAuth failed: ${r.status}${text ? ` ${text.slice(0, 160)}` : ''}`);
  }
  return r.json();
}

export async function exchangeStravaAuthorizationCode(code, redirectUri) {
  return postStravaOAuth(
    new URLSearchParams({
...
export async function refreshTokenIfNeeded() {
  ...
  if (!process.env.STRAVA_REFRESH_TOKEN) throw new Error('missing Strava OAuth env: STRAVA_REFRESH_TOKEN');
  const j = await refreshStravaToken(process.env.STRAVA_REFRESH_TOKEN);
...
export async function getAccessTokenForRequest(req, res, opts = {}) {
  const session = getStravaSession(req);
  if (session) {
    const now = Math.floor(Date.now() / 1000);
    const exp = Number(session.expiresAt || 0);
    if (session.accessToken && exp > now + 120) {
      return { token: session.accessToken, authMode: 'oauth-session', session };
    }

    const refreshed = await refreshStravaToken(session.refreshToken);
    const updated = setStravaSession(req, res, {
      ...refreshed,
      scope: refreshed.scope || session.scope,
      athlete: refreshed.athlete || session.athlete
    });
    return { token: refreshed.access_token, authMode: 'oauth-session', session: updated };
  }

  const allowServerFallback = opts.allowServerFallback || process.env.STRAVA_ALLOW_SERVER_FALLBACK === 'true';
  ...
  return { token, authMode: 'server-env', session: null };
}
...
export function sortActivitiesNewestFirst(activities) {
  return [...activities].sort((a, b) => getActivityTime(b) - getActivityTime(a));
}

export function filterMinimumDistance(activities, minDistanceKm = 0) {
  const minDistanceMeters = Math.max(0, Number(minDistanceKm || 0) * 1000);
  if (!minDistanceMeters) return activities;
  return activities.filter((activity) => getActivityDistanceMeters(activity) >= minDistanceMeters);
}
