...
export function buildLiveCoachingDecision(metrics, opts = {}) {
  ...
  const cadence = Number(metrics.cadence ?? 0);
  const gapSec = Number(metrics.gapSec ?? 0);
  ...
  if (hr) {
    if (hr >= maxHr) {
      ...
      if (elapsedSec >= hrAlertSustained) {
        hrCue += ' 고심박 지속으로 운동 중단(Stop) 권고. 흉통/어지럼/호흡곤란 시 즉시 도움을 요청하세요.';
        action = 'stop';
        severity = 'alert';
      }
  ...
  // 케이던스 코칭 (이상적: 170~180 spm)
  let cadenceCue = '';
  if (cadence > 0) {
    if (cadence < 160) cadenceCue = `케이던스 ${Math.round(cadence)}spm — 보폭 줄이고 회전수 높여요.`;
    else if (cadence > 190) cadenceCue = `케이던스 ${Math.round(cadence)}spm — 약간 보폭 늘려도 괜찮아요.`;
  }

  // GAP 코칭 (경사 보정 페이스)
  let gapCue = '';
  if (gapSec > 0 && paceSec > 0) {
    const gapDiff = gapSec - paceSec;
    if (gapDiff >= 20) gapCue = `오르막 구간 — GAP ${secToPace(gapSec)} 기준 잘 유지 중이에요.`;
    else if (gapDiff <= -20) gapCue = `내리막 — 속도에 주의, GAP ${secToPace(gapSec)}.`;
  }
  ...
  return {
    text: `실시간 코칭: ${paceCue} ${hrCue} ${cadenceCue} ${gapCue} ${phaseCue} ${readinessCue}`.replace(/\s+/g, ' ').trim(),
    severity,
    action,
    nextCheckSec: Number(opts.nextCheckSec ?? 90),
    adjustedTargetPaceSec: targetPaceSec
