...
export function buildLiveCoaching(metrics, opts = {}) {
  const targetPaceSec = Number(opts.targetPaceSec ?? 370);
  ...
  if (!paceSec) {
    return '실시간 코칭: 페이스 데이터가 아직 부족해요. 30초 더 달린 뒤 다시 체크해요.';
  }

  const diff = paceSec - targetPaceSec;
  let paceCue;
  if (diff <= -20) paceCue = `지금 페이스가 빠릅니다 (${secToPace(paceSec)}). 10~15초 늦춰서 ${secToPace(targetPaceSec)} 근처로.`;
  else if (diff >= 25) paceCue = `지금은 살짝 느립니다 (${secToPace(paceSec)}). 상체 세우고 보폭보다 케이던스로 +5~10초 당겨요.`;
  else paceCue = `좋아요. 목표 페이스 범위(${secToPace(targetPaceSec)}±10초) 유지 중입니다.`;

  let hrCue = '';
  if (hr) {
    if (hr >= maxHr) hrCue = `심박 ${hr}로 높은 편이라 1분 완화 조깅 권장.`;
    else if (hr <= maxHr - 25 && elapsedSec > 300) hrCue = `심박 ${hr}로 여유 있어요. 마지막 구간에서 소폭 가속 가능합니다.`;
    else hrCue = `심박 ${hr} 안정.`;
  }
  ...
  return `실시간 코칭: ${paceCue} ${hrCue} ${phaseCue}`.replace(/\s+/g, ' ').trim();
}
